...
const EmptyState = ({ hasFilters, noDataLabel, onClear, t }) => (
  <div className="flex flex-col items-center gap-2 py-12 text-gray-400">
    ...
    <p className="font-semibold text-sm text-center">
      {hasFilters ? t("userManagement.noResultsMatchFilters") : noDataLabel}
    </p>
    {hasFilters && (
      <button
        onClick={onClear}
        ...
        <div className="flex flex-wrap gap-2">
          {STATUS_OPTS.map((s) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${isActive ? "bg-white" : s === "ACTIVE" ? "bg-green-500" : s === "INACTIVE" ? "bg-red-500" : "bg-gray-300"}`}
                />
                {s || t("all")}
              </button>
            );
          ...
          <div className="flex flex-wrap gap-2">
            {["", ...planOptions].map((p) => {
              ...
              return (
                <button
                  key={p || "all-p"}
                  onClick={() => setSubFilter(p)}
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition active:scale-95
                    ${isActive ? "bg-[#800000] text-white border-[#800000]" : "bg-gray-50 text-gray-600 border-gray-200 hover:border-[#800000] hover:text-[#800000]"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${isActive ? "bg-white" : p ? "bg-[#800000]/40" : "bg-gray-300"}`}
                  />
                  {p || t("all_plans")}
                </button>
              );
    ...
    async (page = 1) => {
      ...
      try {
        ...
        if (res.success) {
          ...
          setActiveUser(
            sorted.filter((u) => u.deviceStatus === "ACTIVE").length,
          );
        }
      } catch (err) {
        console.error(err);
        ...
        <div className="shrink-0">
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            {t("userManagement.manage_members") ||
              "Search by MAC address or Device ID"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 lg:gap-3">
          ...
          <div className="flex items-center rounded-xl overflow-hidden border border-gray-200 bg-white shadow-sm flex-1 min-w-0 lg:max-w-sm">
            <div className="relative flex-1 min-w-0">
              ...
              <input
                type="text"
                placeholder={
                  t("userManagement.search_device") ||
                  "Search by MAC or Device ID…"
                }
                value={search}
          ...
          <div className="relative shrink-0" ref={desktopFilterRef}>
            ...
            >
              ...
              <ChevronDown
                size={12}
                className={`hidden md:block transition-transform duration-200 ${showFilters ? "rotate-180" : ""}`}
              />
            </motion.button>
        ...
        {hasFilters && (
          ...
          >
            <span className="text-xs text-gray-500 font-medium">
              {loadingData
                ? "Searching…"
                : `${totalUser} result${totalUser !== 1 ? "s" : ""}`}
            </span>
            {debouncedSearch && (
                ...
                >
                  ...
                  <td className="px-3 py-3.5">
                    <div className="flex items-center justify-center gap-1.5">
                      <span
                        className="font-mono text-xs font-semibold text-gray-700 tracking-wide whitespace-nowrap"
                        title={item.macAddress}
                      >
                        {item.macAddress || "—"}
                      </span>
                      {item.macAddress && (
                        <span className="shrink-0">
                          <CopyButton
                            ...
                            copyLabel={t("userManagement.copy") || "Copy"}
                            copiedLabel={
                              t("userManagement.copied") || "Copied!"
                            }
                          />
                        </span>
                      )}
                  ...
                  <td className="px-3 py-3.5">
                    <div className="flex items-center justify-center gap-1.5 min-w-0">
                      <span
                        className="text-[#800000] font-semibold text-xs truncate min-w-0"
                        title={item.deviceId}
                      >
              ...
              <tr>
                <td colSpan="7" className="py-12">
                  <EmptyState
                    ...
                    noDataLabel={t("userManagement.no_user_found")}
                    onClear={() => {
                      setSearch("");
                      clearFilters();
                    }}
      ...
      {showPagination && (
        <div className="flex justify-center items-center gap-3 p-3 flex-wrap">
          ...
          <span className="text-sm font-semibold text-gray-700">
            {t("userManagement.page")} {currentPage} {t("userManagement.of")}{" "}
            {totalPages}
          </span>
          <button
