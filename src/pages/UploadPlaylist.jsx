...
const PlaylistManager = () => {
  ...
  const [links, setLinks] = useState([
    { id: 1, source: 'Link', url: '', name: '' },
  ]);

  const [isSaving, setIsSaving] = useState(false);
  const [isSaved, setIsSaved] = useState(false);

  // NEW: GET API loading state
  const [isLoading, setIsLoading] = useState(false);

  // ───────────────────────────────────────────────────────────
  // GET DEVICE / PLAYLIST DATA
  // ───────────────────────────────────────────────────────────
  const fetchPlaylistData = async (deviceId, devicePin) => {
    if (!deviceId || !devicePin) {
      console.log('Missing deviceId or pin:', {
        deviceId,
        pin: devicePin,
      });
      return;
    ...
    try {
      console.log('GET PLAYLIST API:', {
        deviceId,
        pin: devicePin,
      });

      const result = await validateDevice(deviceId, devicePin);

      console.log('GET PLAYLIST RESPONSE:', result);

      if (!result.success) {
        console.error('GET PLAYLIST ERROR:', result.message);
        setIsLoading(false);
        return;
      }

      /*
       * Backend response ko safely handle kar rahe hain.
       *
       * Possible responses:
       *
       * 1. { success: true, data: [...] }
       * 2. { success: true, data: { playlists: [...] } }
       * 3. { data: [...] }
       * 4. direct array
       */
      ...
      if (Array.isArray(responseData)) {
        ...
        playlistData = responseData.data;
      } else if (Array.isArray(responseData?.playlists)) {
        playlistData = responseData.playlists;
      } else if (Array.isArray(responseData?.data?.playlists)) {
        playlistData = responseData.data.playlists;
      } else if (
        responseData?.data &&
        typeof responseData.data === 'object'
      ) {
        // Agar single playlist object hai
        if (
          responseData.data.m3uUrl ||
      ...
      console.log('NORMALIZED PLAYLIST DATA:', playlistData);

      if (playlistData.length > 0) {
        const formattedLinks = playlistData.map((item, index) => ({
          id: item.id || item._id || `api-${index}-${Date.now()}`,
        ...
        // Existing API data ko saved state maan rahe hain
        setIsSaved(true);
      } else {
        console.log('No playlist data found in API response');

        // Agar API mein data nahi hai to blank row rakho
        setLinks([
          {
            id: 1,
            source: 'Link',
  ...
  useEffect(() => {
    ...
    const currentMac = macFromState || savedMac || '';
    const currentPin = pinFromState || savedPin || '';
    ...
    if (pinFromState) {
      setPin(pinFromState);
      localStorage.setItem('pin', pinFromState);
    } else if (savedPin) {
      setPin(savedPin);
    }

    // GET API call
    if (currentMac && currentPin) {
      fetchPlaylistData(currentMac, currentPin);
    } else {
      console.log('Cannot call playlist GET API:', {
  ...
  const handleSave = async () => {
    ...
    const result = await saveM3uPlaylist(
      macAddress,
      {
        name: firstLink.name,
    ...
    if (result.success) {
      toast.success(
        result.message || t('playlist.saved')
      );

      setIsSaved(true);

      // Save ke baad latest data dobara GET kar lo
      if (pin) {
