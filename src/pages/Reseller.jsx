...
const tiers = [
  { qty: "10", unitPrice: 2.5, fixedTotal: 25, fixedQty: 10 },
  { qty: "10 – 50", unitPrice: 2.2, fixedTotal: null, fixedQty: null },
  { qty: "50 – 100", unitPrice: 2.0, fixedTotal: null, fixedQty: null },
  { qty: "100 – 200", unitPrice: 1.75, fixedTotal: null, fixedQty: null },
  { qty: "200 – 500", unitPrice: 1.5, fixedTotal: null, fixedQty: null },
...
const Reseller = () => {
  ...
  const updateRowQty = (i, val) =>
    setRowQtys((prev) => ({ ...prev, [i]: val }));
      ...
      <div className="bg-[#f4f4f7] min-h-screen pb-[72px]">
        ...
        <section className="max-w-2xl mx-auto px-4 pt-10 sm:pt-14">
          ...
          >
            ...
            <motion.h3
              variants={fadeUp}
              className="text-xl sm:text-2xl font-extrabold text-[#1a1a1a] mb-3 tracking-tight"
            >
            ...
            <motion.p
              variants={fadeUp}
              className="text-gray-500 text-sm sm:text-base leading-relaxed mb-7 max-w-lg mx-auto"
            >
        ...
        <section className="max-w-4xl mx-auto px-4 mt-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              {
                icon: <Users size={20} className="text-[#800000]" />,
                label: t("resellerpage.reseller_network"),
              },
              {
                icon: <TrendingUp size={20} className="text-[#800000]" />,
                label: t("resellerpage.volume_discounts"),
              },
              {
                icon: <Star size={20} className="text-[#800000]" />,
                label: t("resellerpage.premium_support"),
              },
            ].map(({ icon, label }, i) => (
              <motion.div
                key={i}
        ...
        <section className="max-w-4xl mx-auto px-4 mb-8">
          <motion.h3
            {...fadeUp}
            className="text-lg font-extrabold text-[#1a1a1a] mb-5 text-center tracking-tight"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            >
              <h3 className="font-extrabold text-[#1a1a1a] text-lg mb-1">
                {t("resellerpage.one_code")}
              </h3>
              <p className="text-sm text-gray-500 mb-4">
                {t("resellerpage.annual_sub")}
              </p>
              ...
              <div className="space-y-2 mb-7">
                {[
                  t("resellerpage.one_year_access"),
                  t("resellerpage.instant_activation"),
                  t("resellerpage.full_support"),
                ].map((f) => (
                  <div
                    key={f}
            ...
            >
              ...
              <h3 className="font-extrabold text-white text-lg mb-1 relative z-10">
                {t("resellerpage.two_codes")}
              </h3>
              <p className="text-sm text-gray-400 mb-4 relative z-10">
                {t("resellerpage.best_couples")}
              </p>
  ...
  {[
    t("resellerpage.two_devices_covered"),
    t("resellerpage.best_value_deal"),
    t("resellerpage.priority_support"),
  ].map((f) => (
    <div
      key={f}
        ...
        <section className="max-w-4xl mx-auto px-4 mb-8">
          ...
          >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-black/[0.06]">
                    <th className="px-6 py-3 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">
                      {t("resellerpage.qty")}
                    </th>
                    <th className="px-5 py-3 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">
                      {t("resellerpage.unit_price")}
                    </th>
                    <th className="px-5 py-3 text-center text-xs font-bold text-gray-400 uppercase tracking-widest">
                      Your Qty
                    </th>
                    <th className="px-6 py-3 text-right text-xs font-bold text-gray-400 uppercase tracking-widest">
                      {t("resellerpage.total_label")}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {tiers.map((tier, i) => {
                    ...
                    return (
                      ...
                      >
                        {/* Qty range */}
                        <td className="px-6 py-3 font-bold text-[#1a1a1a] whitespace-nowrap">
                          {tier.qty}
                        </td>
            ...
            <div className="px-6 py-4 bg-[#800000]/[0.03] border-t border-[#800000]/10 space-y-1.5">
              {[
                t("resellerpage.bonus_msg"),
                t("resellerpage.whatsapp_msg"),
              ].map((msg, i) => (
                <div
                  key={i}
                  className="flex items-start gap-2 text-xs text-gray-600 font-semibold"
                >
                  <CheckCircle2
                    size={13}
                    className="text-[#800000] shrink-0 mt-0.5"
                  />
                  {msg}
                </div>
              ))}
