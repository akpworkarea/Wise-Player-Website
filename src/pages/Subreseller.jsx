    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {STATUS_OPTS.map((s) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${isActive ? "bg-white" : s.dot}`}
                />
                {s.label}
              </button>
            );
      ...
      {activeFilterCount > 0 && (
        <div className="pt-3 border-t border-gray-100">
          ...
          >
            {t("resellerpage.Clearallfilters")}
          </button>
        </div>
      )}
    </div>
...
const StatusBadge = ({ active }) => (
  ...
  >
    <span
      className={`w-1.5 h-1.5 rounded-full shrink-0 ${active ? "bg-green-500" : "bg-red-500"}`}
    />
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <span className="font-bold text-sm text-gray-800 truncate min-w-0 pr-1">
          {user.fullName}
        </span>
...
const EmptyState = ({ hasFilters, noDataLabel, onClear, t }) => (
  <div className="flex flex-col items-center gap-2 py-12 text-gray-400">
    ...
    <p className="font-semibold text-sm text-center">
      {hasFilters ? t("resellerpage.noSubResellersMatchFilters") : noDataLabel}
    </p>
    {hasFilters && (
      <button
        onClick={onClear}
    ...
    <div className="min-h-screen w-full bg-[#f4f4f7] p-4 space-y-5">
          ...
          <div className="flex items-center rounded-xl overflow-hidden border border-gray-200 bg-white shadow-sm flex-1 min-w-0 lg:max-w-sm">
            <div className="relative flex-1 min-w-0">
              ...
              <input
                type="text"
                placeholder={
                  t("search_Placeholder") || "Search by name, username or ID…"
                }
                value={search}
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-gray-800">
                        {t("resellerpage.FilterSubResellers")}
                      </span>
                    </div>
                    <button
          ...
          <thead>
            <tr className="bg-gray-100 border-b border-gray-200">
              {[
                ...
                <th
                  key={col}
                  className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider text-gray-600 first:text-left"
                >
                  {col}
                </th>
              ))}
      ...
      <div className="hidden md:grid lg:hidden grid-cols-2 gap-3">
        {loadingData ? (
          ...
          users.map((user) => (
            <SubCard key={user.id} user={user} {...cardProps} />
          ))
        ) : (
      ...
      {showPagination && (
        <div className="flex justify-center items-center gap-3 p-3 flex-wrap">
          ...
          <span className="text-sm font-semibold text-gray-700">
            {t("transaction.page") || "Page"} {currentPage}{" "}
